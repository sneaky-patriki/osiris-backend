...
void IntListInsertInOrder (IntList L, int v)
{
	...
	IntListNode *currentNode = L->first;
	IntListNode *newElement = newIntListNode(v);
	L->size++;

	// Case: element becomes inserted at the start of the list
	...
	// Case: there is only 1 element in the list and the value is inserted after it
	if (currentNode->next == NULL && currentNode->data <= v) {
		currentNode->next = newElement;
		L->last = newElement;
		return;
	}

	while (currentNode->next->data <= v && currentNode->next->next != NULL) {
		currentNode = currentNode->next;
	}

	// Case: the value is added to the end of the list
	if (currentNode->next->next == NULL && currentNode->next->data <= v) {
		currentNode->next->next = newElement;
		L->last = newElement;
	} else {
		// Case: the value is inserted somewhere in the list
